...
//RECURSION
//Read with explicitly defined recursion number
exprt path_symex_statet::read(const exprt &src, bool propagate, unsigned recursion_number)
{
  ...
  // we force propagation for dereferencing
    // std::cout<<"Going for Derefernce : "<<from_expr(var_map.ns, "",src)<<"\n";
  exprt tmp3=dereference_rec(src, true, recursion_number);
  // std::cout<<"Dereferencing Result : "<<from_expr(var_map.ns, "", tmp3)<<"\n";
  exprt tmp4=instantiate_rec(tmp3, propagate, recursion_number);
  // std::cout<<"instantiate_rec Result : "<<from_expr(var_map.ns, "",tmp4)<<"\n";
  exprt tmp5=simplify_expr(tmp4, var_map.ns);
  // std::cout<<"simplify_expr Result : "<<from_expr(var_map.ns, "", tmp5)<<"\n\n";
...
//RECURSION
//Mostly for passing the recursion_number down to read_symbol_member_index
//Is there a 
exprt path_symex_statet::instantiate_rec(
  const exprt &src,
  bool propagate,
  unsigned recursion_number)
{
  ...
  if(src.id()==ID_address_of)
  {
    assert(src.operands().size()==1);
    exprt tmp=src;
    tmp.op0()=instantiate_rec_address(tmp.op0(), propagate, recursion_number);
    return tmp;
  }
  else if(src.id()==ID_side_effect)
  {
  ...
  Forall_operands(it, src2)
  {
    exprt tmp_op=instantiate_rec(*it, propagate, recursion_number);
    *it=tmp_op;
  }
  ...
  unsigned recursion_number)
{
  ...
  {
    Forall_operands(it, final)
      *it=read_symbol_member_index(*it, propagate,recursion_number); // rec. call
    return final;
  }
  ...
  {
    assert(final.operands().size()==3);
    final.op0()=instantiate_rec(final.op0(), propagate, recursion_number); // rec. call
    final.op1()=read_symbol_member_index(final.op1(), propagate, recursion_number); // rec. call
    final.op2()=read_symbol_member_index(final.op2(), propagate, recursion_number); // rec. call
    return final;
  }
  ...
  //TODO: Is this actually needed here?
    //Surely, not for return statements. What about arguments?
  //RECURSION
  //To handle pointers after they have been dereferenced
  //The dereferenced variable 's recursion number should be used.
  // path_symex_statet::threadt &thread=threads[get_current_thread()];
  ...
  unsigned recursion_number)
{
...

  {
    ...
    Forall_operands(it, src2)
    {
      exprt tmp_op=dereference_rec(*it, propagate, recursion_number);
      *it=tmp_op;
    }
  }
  ...
  unsigned recursion_number)
{
  ...
  else if(src.id()==ID_index)
  {
    assert(src.operands().size()==2);
    exprt tmp=src;
    tmp.op0()=instantiate_rec_address(src.op0(), propagate, recursion_number);
    tmp.op1()=instantiate_rec(src.op1(), propagate, recursion_number);
    return tmp;
  }
  else if(src.id()==ID_dereference)
  {
    ...
    // which we simply instantiate as non-address
    dereference_exprt tmp=to_dereference_expr(src);
    tmp.pointer()=instantiate_rec(tmp.pointer(), propagate, recursion_number);
    return tmp;
  }
  else if(src.id()==ID_member)
  {
    member_exprt tmp=to_member_expr(src);
    tmp.struct_op()=instantiate_rec_address(tmp.struct_op(), propagate, recursion_number);
    return tmp;
  }
  else if(src.id()==ID_string_constant)
  {
  ...
  else if(src.id()==ID_if)
  {
    if_exprt if_expr=to_if_expr(src);
    if_expr.true_case()=
      instantiate_rec_address(if_expr.true_case(), propagate, recursion_number);
    if_expr.false_case()=
      instantiate_rec_address(if_expr.false_case(), propagate, recursion_number);
    if_expr.cond()=instantiate_rec(if_expr.cond(), propagate, recursion_number);
    return if_expr;
  }
  else if(src.id()=="dereference_error")
  {
